...
const Login = () => {
  ...
  const handleChange = (e) => {
    ...
    state === 'login' ? setLoginForm(prev => ({ ...prev, [name]: value })) : setRegisterForm(prev => ({ ...prev, [name]: value }))
  }
  ...
  return (
    <form onSubmit={handleFormSubmit} className="sm:w-87.5 w-full text-center border border-gray-300/60 rounded-2xl px-8 bg-white">
      ...
      {state !== "login" && (
        <div className="flex items-center mt-6 w-full bg-white border border-gray-300/80 h-12 rounded-full overflow-hidden pl-6 gap-2">
          ...
          <input type="text" name="name" placeholder="Name" className="border-none outline-none ring-0" value={loginForm.name} onChange={handleChange} required />
        </div>
      )}
      <div className="flex items-center w-full mt-4 bg-white border border-gray-300/80 h-12 rounded-full overflow-hidden pl-6 gap-2">
        ...
        <input type="email" name="email" placeholder="Email id" className="border-none outline-none ring-0" value={state === 'login' ? loginForm.email : registerForm.email} onChange={handleChange} required />
      </div>
      <div className="flex items-center mt-4 w-full bg-white border border-gray-300/80 h-12 rounded-full overflow-hidden pl-6 gap-2">
        ...
        <input type="password" name="password" placeholder="Password" className="border-none outline-none ring-0" value={state === 'login' ? loginForm.password : registerForm.password} onChange={handleChange} required />
      </div>
